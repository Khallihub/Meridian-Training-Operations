...
class TestLogin:
    async def test_login_success(self, client, db):
        ...
        resp = await client.post("/api/v1/auth/login", json={"username":"test_admin", "password":"TestPass@1234"})
    ...
    async def test_login_wrong_password(self, client, db):
        ...
        resp = await client.post("/api/v1/auth/login", json={"username":"user_wrong_pw", "password":"WrongPass@1"})
    ...
    async def test_account_lockout_after_5_failures(self, client, db):
        ...
        for _ in range(5):
            await client.post("/api/v1/auth/login", json={"username":"lockout_user", "password":"Bad@Pass1234"})
        resp = await client.post("/api/v1/auth/login", json={"username":"lockout_user", "password":"Bad@Pass1234"})
...
class TestProtectedEndpoints:
    ...
    async def test_authenticated_returns_user(self, client, db):
        ...
        token_resp = await client.post("/api/v1/auth/login", json={"username":"me_user", "password":"TestPass@1234"})
        token = resp_data(token_resp)["access_token"]
        resp = await client.get("/api/v1/users/me", headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
        assert resp_data(resp)["username"] == "me_user"
...
class TestChangePassword:
    async def test_change_password_success(self, client, db):
        ...
        token_resp = await client.post("/api/v1/auth/login", json={"username":"pw_changer", "password":"TestPass@1234"})
        token = resp_data(token_resp)["access_token"]
    ...
    async def test_change_password_wrong_current(self, client, db):
        ...
        token_resp = await client.post("/api/v1/auth/login", json={"username":"pw_fail_user", "password":"TestPass@1234"})
        token = resp_data(token_resp)["access_token"]
        resp = await client.post(
            "/api/v1/auth/change-password",
