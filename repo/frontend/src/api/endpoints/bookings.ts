...
export const bookingsApi = {
  ...
  create: async (sessionId: string) => {
    const res = await client.post('/api/v1/bookings', { session_id: sessionId })
    return res.data as Booking
  },
  confirm: async (id: string) => {
    const res = await client.patch(`/api/v1/bookings/${id}/confirm`)
    return res.data as Booking
  },
  reschedule: async (id: string, newSessionId: string) => {
    const res = await client.post(`/api/v1/bookings/${id}/reschedule`, { new_session_id: newSessionId })
    return res.data as Booking
  },
  ...
  getHistory: async (id: string) => {
    const res = await client.get(`/api/v1/bookings/${id}/history`)
