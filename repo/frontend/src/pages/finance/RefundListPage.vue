...
const ui = useUiStore()
const activeTab = ref<'pending' | 'approved' | 'completed' | 'rejected'>('pending')
...
async function handleApprove(id: string) {
  try {
    await payments.approveRefund(id)
    ui.addToast('Refund approved', 'success')
    loadRefunds()
...
async function handleComplete(id: string) {
  try {
    await payments.completeRefund(id)
    ui.addToast('Refund completed', 'success')
    loadRefunds()
  } catch (e: any) { ui.addToast(e.message, 'error') }
}
...
<template>
  <AppLayout>
    ...
    <div class="flex gap-2 mb-4 border-b border-border">
      <button v-for="tab in ['pending','approved','completed','rejected']" :key="tab"
        @click="() => { activeTab = tab as any; loadRefunds() }"
        :class="['pb-2 px-3 text-sm font-medium border-b-2 -mb-px transition-colors', activeTab === tab ? 'text-primary border-primary' : 'text-muted-foreground border-transparent hover:text-foreground']"
          ...
          <tr v-for="r in payments.refunds" :key="r.id" class="border-t border-border hover:bg-muted/30">
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex justify-end gap-2">
                <button v-if="r.status === 'pending'" @click="handleApprove(r.id)" class="px-2 py-1 text-xs bg-green-600 text-white rounded hover:bg-green-700">Approve</button>
                <button v-if="r.status === 'approved'" @click="handleComplete(r.id)" class="px-2 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700">Complete</button>
              </div>
            </td>
          </tr>
        </tbody>
